...
const ChatComponent = () => {
  ...
  const handleSend = async () => {
    if (input.trim()) {
      ...
      try {
        const response = await axios.post("http://localhost:8080/query", input, {
        ...
        setMessages((prev) => [...prev, { id: messages.length + 2, text: "Error: Unable to get a response", sender: 'ai' }]);
      } finally {
        setLoading(false); 
      }
    }
    ...
    <Box sx={{ display: 'flex', height: '100vh', backgroundColor: '#f0f0f0' }}>
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <Card sx={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: '#ffffff' }}>
          <CardHeader title="GenAI Customer Support" sx={{ paddingBottom: 0 }}/>
